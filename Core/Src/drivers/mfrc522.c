...
#define MFRC_ADDR_MODE_Msk  (0x1 << 6)
#define MFRC_ADDR_SET_READ(addr)    ((addr | MFRC_ADDR_MODE_Msk) << 1U) // set 6th bit (mode) shift to clear LSB and move mode to 7th bit
#define MFRC_ADDR_SET_WRITE(addr)   ((addr & ~MFRC_ADDR_MODE_Msk) << 1U) // clear 6th bit (mode) shift to clear LSB and move mode to 7th bit
...
void mfrc522_init(void) {
    ...
    uint8_t value = 0x8D; //
    write_mfrc_register(MFRC_TMODER, &value, SINGLE_REG_WRITE);
    value = 0x3E; //
    write_mfrc_register(MFRC_TPSCR, &value, SINGLE_REG_WRITE);
    value = 30; // timer count to 30 ticks
    write_mfrc_register(MFRC_TRELOADR_L, &value, SINGLE_REG_WRITE);
    value = 0; // high bits of timer reload cleared
    write_mfrc_register(MFRC_TRELOADR_H, &value, SINGLE_REG_WRITE);

    value = 0x70; // 48dB gain
    write_mfrc_register(MFRC_RFCfgR, &value, SINGLE_REG_WRITE);
...
uint8_t mfrc522_auth(uint8_t auth_type, uint8_t picc_block_addr, uint8_t *sector_key, uint8_t *serial_num) {
    ...
    while (retries > 0) {
        read_mfrc_register(MFRC_STAT2_REG, &status2_reg, SINGLE_REG_READ);
 ...
 */
static uint8_t mfrc_picc_read(uint8_t picc_block_addr, uint8_t *rcv_data) {
    mfrc_status_e status = MFRC_OK;
    uint8_t buffer[4];
    uint16_t rcv_len;
...
static uint8_t mfrc_calculate_crc(uint8_t *checksum_data, uint8_t data_len, uint8_t *checksum) {
    ...
    write_mfrc_register(MFRC_FIFO_DR, checksum_data, data_len);
    ...
    while (retries > 0) {
        read_mfrc_register(MFRC_DIV_IRQ, &irq_status, SINGLE_REG_READ);
    ...
    read_mfrc_register(MFRC_CRC_RESR_MSB, checksum, SINGLE_REG_READ); // read 8 MSB into checksum[0:7]
    read_mfrc_register(MFRC_CRC_RESR_LSB, (checksum + 8), SINGLE_REG_READ); // read 8 LSB into checksum[8:15] 
...
static uint8_t mfrc_anticollision(uint8_t *uid) {
    ...
    write_mfrc_register(MFRC_BIT_FRAMING, &val, SINGLE_REG_WRITE);
...
static uint8_t mfrc_antenna_on(void) {
    uint8_t curr_reg = 0;
    uint8_t rslt = read_mfrc_register(MFRC_TX_CTLR, &curr_reg, SINGLE_REG_READ);
    if (!(curr_reg & 0x03)) {
        uint8_t value = 0x03; // activate 13.56MHz signal on TX1/2 pins
        rslt |= write_mfrc_register(MFRC_TX_CTLR, &value, SINGLE_REG_WRITE);
    }
    return rslt;
}
...
static uint8_t mfrc_send_to_picc(uint8_t command, uint8_t *send_data, uint8_t send_len, uint8_t *rcv_data, uint16_t *rcv_len) {
    ...
    switch (command) {
        ...
        write_mfrc_register(MFRC_COM_INT_EN, &irq_en, SINGLE_REG_WRITE);
        ...
        rslt |= write_mfrc_register(MFRC_FIFO_DR, send_data, send_len); // VERIFY THAT READER ALLOWS SEQUENTIAL WRITES
        ...
        while (retries > 0) {
            rslt |= read_mfrc_register(MFRC_COM_IRQ, &irq_status, SINGLE_REG_READ);
        ...
        if (retries != 0) {
            ...
            rslt |= read_mfrc_register(MFRC_ERR_REG, &err_check, SINGLE_REG_READ);
            if (!(err_check & 0x1B)) { // check for RX buffer overflow, collision err, or RX CRC err
                ...
                if (command == PCD_CMD_TRANSCEIVE) { // read data bytes from card
                    ...
                    read_mfrc_register(MFRC_FIFO_LVL, &num_bytes, SINGLE_REG_READ);
                    num_bytes = num_bytes & 0x7F; // only lower 6 bits tell us how many bytes are in DR
                    read_mfrc_register(MFRC_CTL_REG, &valid_bits, SINGLE_REG_READ);
...
static uint8_t set_bitmask_on_reg(uint8_t reg, uint8_t reg_msk) {
    ...
    uint8_t rslt = read_mfrc_register(reg, &curr_reg_sttg, SINGLE_REG_READ);
    if (rslt)
        return 1;
    // mask specified bits
