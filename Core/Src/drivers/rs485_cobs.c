...
static uint16_t rs485_cobs_decode(uint8_t *encoded_buf, uint32_t length, void *data);

// STATIC TX BUFFER INFO
STATIC_RING_BUFFER(frame_rb, MAX_MESSAGES, serial_t);
static serial_t active_tx_buf = {0};
static volatile uint8_t tx_ongoing = FALSE;

// STATIC RECEIVE BUFFER INFO
STATIC_RING_BUFFER(rx_frame_rb, MAX_MESSAGES, serial_t);
static uint16_t active_rx_idx = 0;
static serial_t active_rx_buf = {0};
...
static void(*msg_ready_cb)(void) = NULL;
